function FullSubtractor(x, y) {
    this.x = x;
    this.y = y;

    this.index = -1;
    this.name = "FULL_SUBTRACTOR_";
    ...
    this.draw = function() {
        ...
        for (i = 0; i < this.out.length; i++) {
            var but = this.out[i];
            if (mouseX > but.x - 10 && mouseX < but.x + 10 && mouseY > but.y - 10 && mouseY < but.y + 10) {
                but.show();
            } else {
                but.hide();
            }
    ...
    this.refresh = function(index) {
        ...
        this.index = index;
        this.name = "FULL_SUBTRACTOR_" + this.index;
